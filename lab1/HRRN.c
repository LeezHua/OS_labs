...
void HRRN(proc_queue ready){
  ...
  while(!empty_queue(ready)){
    ...
    qnode_ptr node = grade_proc(ready);
...
int main(){
  ...
  set_proc(procs[0], 2, 0, 14, 1);
  set_proc(procs[1], 8, 0, 9 , 1);
  set_proc(procs[2], 7, 0, 2 , 1);
  set_proc(procs[3], 3, 0, 8 , 1);
  set_proc(procs[4], 5, 0, 4 , 1);
  set_proc(procs[5], 1, 0, 5 , 1);
